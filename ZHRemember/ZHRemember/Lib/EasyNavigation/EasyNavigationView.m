...
- (void)layoutNavSubViews
{
    ...
    CGFloat normalHeight = NavigationNorlmalHeight_N();
    //如果是iPhone X的横屏状态，让出安全区域的距离
    CGFloat leftMagrin = [EasyNavigationOptions shareInstance].leftMargin;
    __block CGFloat leftEdge = leftMagrin + ((IsIphoneX_N()&&ScreenIsHorizontal_N())? 20 : 0);
    [self.leftViewArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        tempView.frame = CGRectMake(leftEdge, subViewY + (normalHeight - tempView.height) / 2, tempView.width , tempView.height);
        leftEdge += tempView.width ;
    }];
    
    CGFloat rightMargin = [EasyNavigationOptions shareInstance].rightMargin;
    __block CGFloat rightEdge = rightMargin + ((IsIphoneX_N()&&ScreenIsHorizontal_N())? 20 : 0) ;
    [self.rightViewArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        tempView.frame = CGRectMake(tempViewX, subViewY + (normalHeight - tempView.height) / 2, tempView.width , tempView.height);
